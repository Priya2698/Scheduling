...
rm -f $LOG_FILE
echo "Switch1 and Switch3 used for the experiment with PPN=1:" >> $LOG_FILE
...
do
    ...
    liveNodes1=()
    liveNodes2=()
    # Getting active nodes in reverse under SWITCH1
    for((i=${#SWITCH1[@]}-1; i>=0; i--))
    do
        if ping -c1 -w1 ${SWITCH1[$i]} &>/dev/null
            then liveNodes1+=(${SWITCH1[$i]})
        fi
    done
    # Getting active nodes in reverse under SWITCH3
    for((i=${#SWITCH3[@]}-1; i>=0; i--))
    do
        if ping -c1 -w1 ${SWITCH3[$i]} &>/dev/null
            then liveNodes2+=(${SWITCH3[$i]})
        fi
    done 
    ...
    nodes2=$(join , ${liveNodes1[@]:0:${#liveNodes1[@]}-4}),$(join , ${liveNodes2[@]:0:${#liveNodes2[@]}-4})
    # Switch3
    ...
    echo "Total Active Nodes under Switch1=${#liveNodes1[@]} and Switch3=${#liveNodes2[@]}"
    ...
    echo "> Operating Nodes across switch 1 and 3 ($NODES) : $nodes2"
    echo "> Operating Nodes across switch 1 and 3 ($NODES) : $nodes2" >> $LOG_FILE
    ...
    do
        ...
        mpiexec -np $NODES -ppn 1 --hosts $nodes2 ./mpi_allgather.o $data_size $RUNS comm_across_$data_size >> $LOG_FILE
